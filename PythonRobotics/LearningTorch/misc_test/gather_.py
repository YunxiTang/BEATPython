...
def test1():
    ...
    print(tensor_0)
    index = torch.tensor([[2, 1, 0], [1, 1, 2]]).long()
...
def test3():
    ...
    selected_indices = torch.tensor([[0, 2], [1, 3], [0, 4]])
...
def test4():
    ...
    tokens = torch.randn(
        3, 5, 4
    )  # 3 sequences, each of length 5, embedding dimension of 4

    # Index tensor (batch_size x num_indices), containing the indices to gather
    indices = torch.tensor(
        [[0, 2], [1, 3], [0, 4]]
    )  # Select 2 tokens for each sequence

    # Use advanced indexing to directly select the tokens
    batch_indices = torch.arange(tokens.size(0)).unsqueeze(
        1
    )  # Create batch indices (batch_size x 1)
