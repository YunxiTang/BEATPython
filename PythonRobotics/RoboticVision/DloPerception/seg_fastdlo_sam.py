...
if __name__ == "__main__":
    ...
    p = Pipeline(
        checkpoint_siam=checkpoint_siam,
        checkpoint_seg=checkpoint_seg,
        img_w=IMG_W,
        img_h=IMG_H,
    )
    ...
    sam = sam_model_registry["vit_h"](
        checkpoint="/home/yxtang/CodeBase/thrid_party/checkpoints/sam_vit_h_4b8939.pth"
    )
    sam.to(device=device)
    predictor = SamPredictor(sam)

    cap = cv2.VideoCapture(
        "/media/yxtang/Extreme SSD/DOM_Reaseach/dobert_cache/dobert_dataset/dom_dataset/episode_1009/episode_1009.mp4"
    )
