...
def forward_noising(xs, noises, ts, sqrt_alpha_bar, one_minus_sqrt_alpha_bar):
    flatten_ts = ts
    reshaped_sqrt_alpha_bar_t = jnp.expand_dims(
        jnp.take(sqrt_alpha_bar, flatten_ts), [1, 2, 3]
    )
    reshaped_one_minus_sqrt_alpha_bar_t = jnp.expand_dims(
        jnp.take(one_minus_sqrt_alpha_bar, flatten_ts), [1, 2, 3]
    )
    noisy_xs = (
        reshaped_sqrt_alpha_bar_t * xs + reshaped_one_minus_sqrt_alpha_bar_t * noises
    )
    return noisy_xs
...
class DDPMScheduler:
    def __init__(self, timesteps: int, seed: int = 0):
        ...
        self.alpha_bar = jnp.concatenate(
            [jnp.array([1.0]), self.alpha_bar[:-1]], axis=0
        )
    ...
    def add_noise(self, xs, noises, steps):
        noisy_xs = forward_noising(
            xs, noises, steps, self.sqrt_alpha_bar, self.one_minus_sqrt_alpha_bar
        )
        return noisy_xs
