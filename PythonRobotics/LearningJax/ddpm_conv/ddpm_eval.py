...
if __name__ == "__main__":
    ...
    for t in tqdm(range(1, scheduler.timesteps, 1)):
        ...
        noise_pred = jitted_model_apply(
            variables, noisy_sample, timesteps, label_conds, False
        )  # predict noise

        noisy_sample = scheduler.backward_denoising_ddpm(
            noisy_sample, noise_pred, timesteps
        )
        noisy_sample = noisy_sample

        if (
            t % 30 == 0
            or (t % 15 == 0 and t > scheduler.timesteps // 3)
            or t == scheduler.timesteps - 1
        ):
            tmp = []
            for i in range(num):
    ...
    if img.mode == "F":
        img = img.convert("RGB")
    img.save(
        "/home/yxtang/CodeBase/PythonCourse/PythonRobotics/LearningJax/ddpm_conv/res/result.jpg"
    )
    save_gif(
        res,
