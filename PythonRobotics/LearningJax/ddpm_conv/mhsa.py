...
def scaled_dot_product(q, k, v):
    d_k = q.shape[-1]
    k_transpose = einops.rearrange(
        k, "batch nhead seqlen dim -> batch nhead dim seqlen"
    )
...
class MHSA(nn.Module):
    ...
    def setup(self):
        self.qkv_proj = nn.Dense(
            3 * self.embed_dim, kernel_init=self.kernel_init, bias_init=self.bias_init
        )
        self.o_proj = nn.Dense(
            self.embed_dim, kernel_init=self.kernel_init, bias_init=self.bias_init
        )
    ...
    def __call__(self, x):
        ...
        values = einops.rearrange(
            values, "batch nhead seqlen dim -> batch seqlen nhead dim"
        )
...
if __name__ == "__main__":
    ...
    outputs, variables = model.init_with_output(
        {"params": jax.random.PRNGKey(12)}, sample
    )
