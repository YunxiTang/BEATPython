...
class CondResConv2D(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        embed = jnp.reshape(
            condition_embed, (condition_embed.shape[0], 2, 1, 1, self.out_channels)
        )
...
class PatchEmbedding(nn.Module):
    ...
    def setup(self):
        ...
        self.conv = nn.Conv(
            self.num_hiddens, kernel_size=patch_size, strides=patch_size, padding=0
        )
...
class CondUnet2D(nn.Module):
    ...
    def __call__(self, x, diffustion_step, condition, train):
        ...
        diffustion_step_embed = nn.Sequential(
            [
                SinusoidalEmbedding(self.diffusion_step_embed_dim),
                nn.Dense(2 * self.diffusion_step_embed_dim),
                Mish(),
                nn.Dense(self.diffusion_step_embed_dim),
            ]
        )(diffustion_step)

        condition_embed = nn.Sequential(
            [
                nn.Dense(2 * self.condition_embed_dim),
                Mish(),
                nn.Dense(self.condition_embed_dim),
            ]
        )(condition)

        global_cond_embed = jnp.concatenate(
            [diffustion_step_embed, condition_embed], axis=-1
        )

        channels = [self.basic_channel * i for i in self.channel_scale_factor]

        # first conv layer
        x = CondResConv2D(self.basic_channel, self.kernel_size, self.num_groups)(
            x, global_cond_embed
        )
        ...
        for down_index, down_channel in enumerate(channels):
            x = CondResConv2D(down_channel, self.kernel_size, self.num_groups)(
                x, global_cond_embed
            )
            x = CondResConv2D(down_channel, self.kernel_size, self.num_groups)(
                x, global_cond_embed
        ...
        x = CondResConv2D(mid_channel, self.kernel_size, self.num_groups)(
            x, global_cond_embed
        )
        x = CondResConv2D(mid_channel, self.kernel_size, self.num_groups)(
            x, global_cond_embed
        ...
        for up_index, up_channel in enumerate(reversed(channels)):
            ...
            x = CondResConv2D(up_channel, self.kernel_size, self.num_groups)(
                x, global_cond_embed
            )
            x = CondResConv2D(up_channel, self.kernel_size, self.num_groups)(
                x, global_cond_embed
        ...
        x = CondResConv2D(self.in_channel, self.kernel_size, num_ng)(
            x, global_cond_embed
        )
        return x
...
if __name__ == "__main__":
    ...
    output, variables = model.init_with_output(
        {"params": jax.random.PRNGKey(0)}, x, cond
    )
    ...
    tc = time.time()
    output, variables = model.init_with_output(
        {"params": jax.random.PRNGKey(0)}, sample, diff_step, cond, False
    )
    et = time.time() - tc
    print("---")
