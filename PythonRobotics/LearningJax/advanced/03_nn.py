...
def loss_dp(params, batch):
    @partial(shard_map, mesh=mesh, in_specs=PartitionSpec('batch', None), out_specs=PartitionSpec())
...
def allclose(a, b):
    return jax.tree_util.tree_all(jax.tree_util.tree_map(partial(jnp.allclose, atol=1e-2, rtol=1e-2), a, b))
