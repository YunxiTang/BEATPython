...
class DummyDataset(Dataset):
    def __init__(self, num_sample:int):
        ...
        self.ys = np.sin(self.xs) + 1.2 * self.xs + np.random.uniform(-0.02, 0.02, size=(num_sample,))
...
data_sharding = NamedSharding(mesh, spec=PartitionSpec('batch',))
...
shard_loss_fn = jax.jit(shard_map(
    get_loss, mesh,
