...
class BaseTrainer:
    ...
    def get_checkpoint_path(self, tag="latest"):
        ckpt_path = pathlib.Path(self._ckpt_dir).joinpath(
            "checkpoints", f"epoch_{tag}.pkl"
        )
        ckpt_path.parent.mkdir(parents=False, exist_ok=True)
        return ckpt_path.absolute()

    def save_checkpoint(
        self, tag: str = "latest", exclude_keys=None, include_keys=None, use_thread=True
    ):
        """
        save checkpoint
        """
        ...
        # Save the checkpoint
        payload = {"cfg": self._cfg, "train_state": self.train_state, "pickles": dict()}
        ...
        if use_thread:
            ...
            self._saving_thread = threading.Thread(
                target=dill.dump, kwargs={"file": f, "obj": payload}
            )
    ...
    def save_snapshot(self, tag="latest"):
        ...
        """
        snapshot_path = pathlib.Path(self._ckpt_dir).joinpath(
            "snapshots", f"snapshot_{tag}.pkl"
        )
