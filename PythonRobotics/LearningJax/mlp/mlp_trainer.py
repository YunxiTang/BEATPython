...
class MLPTrainer(BaseTrainer):
    ...
    def create_function(self):
        def loss_fn(params, state: TrainState, batch, train: bool):
            ...
            loss_val = jnp.mean(
                optax.l2_loss(predicts, labels)
            )  # jnp.mean(predicts, labels)
    ...
    def run(self, cfg=None):
        ...
        # setup dataset
        x = jnp.linspace(0.0, 10.0, 1000)[..., None]
        y = 2 * x + jax.random.normal(jax.random.PRNGKey(0), (1000, 1)) * 0.01
        train_dataloader = FlaxDataloader(
            x, y, cfg.train.batch_size, jax.random.PRNGKey(12)
        )
