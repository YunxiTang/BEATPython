...
class SimpleNode:
    ...
    def __repr__(self) -> str:
        return (
            "Simple Node" + f" at {self._state} obstacles" + f" with {self._path_len}"
        )
...
class RRT:
    ...
    def _get_nearest_node(self, rand_node):
        dlist = [
            RRT._compute_node_distance(rand_node, node) for node in self._node_list
        ]
...
if __name__ == "__main__":
    ...
    world_map = TwoDimMap([0.0, 2.0, 0.0, 2.0], resolution=0.02)
    ...
    if not world_map.check_pos_collision(start) and not world_map.check_pos_collision(
        goal
    ):
        rrt = RRT(
            start_config=start,
