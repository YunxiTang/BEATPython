...
class Visualizer:
    def __init__(self, world_map, path_solution=None, path_list=None) -> None:
        ...
        if path_list is not None:
            ...
            for i in range(num_path):
                self.path_pub_list.append(
                    rospy.Publisher(f"optimal_path{i}", Path, queue_size=1)
                )
        ...
        marker.scale.x = 200.0
        marker.scale.y = 200.0
        marker.scale.z = 0.5
    ...
    def visualize_with_plane(self):
        boundaryPolygon = rospy.Publisher(
            "Passage_Polygon", PolygonStamped, queue_size=1
        )
