...
class CityMap(ABCMap):
    ...
    def __init__(self, start=None, goal=None, resolution: float = 0.1):
        ...
        # add walls
        wall_left = Block(5.0, 200.0, 200.0, -2.5, 100.0, is_wall=True)
        wall_right = Block(5.0, 200.0, 200.0, 202.5, 100.0, is_wall=True)
        wall_down = Block(200.0, 5.0, 200.0, 100, -2.5, is_wall=True)
        wall_up = Block(200.0, 5.0, 200.0, 100, 202.5, is_wall=True)
    ...
    def sample_free_pos(self, toward_goal: bool = False) -> jnp.ndarray:
        ...
        if toward_goal and self._goal is not None:
            ...
            while True:
                self._rng_key, rng_key_x, rng_key_y, rng_key_z = random.split(
                    self._rng_key, 4
                )
                x = random.uniform(
                    rng_key_x, shape=(1,), minval=self._xmin, maxval=self._xmax
                )
                y = random.uniform(
                    rng_key_y, shape=(1,), minval=self._ymin, maxval=self._ymax
                )
                z = random.uniform(
                    rng_key_z, shape=(1,), minval=self._zmin, maxval=self._zmax
                )
    ...
    def visualize_map(self, ax=None):
        ...
        if self._goal is not None:
            ax.scatter(
                self._goal[0], self._goal[1], self._goal[2], color="r", marker="*", s=85
            )
    ...
    def sample_free_pos(self, toward_goal: bool = False) -> jnp.ndarray:
        if toward_goal and self._goal is not None:
            ...
            while True:
                ...
                sampled_pos = random.uniform(
                    rng_key, shape=(self._dim,), minval=self._lb, maxval=self._ub
                )
                collision = self.check_pos_collision(sampled_pos)
                if not collision:
                    return sampled_pos
...
if __name__ == "__main__":
    ...
    city_map = CityMap(start=np.array([0.0, 0.0, 0.0]), goal=[200.0, 200.0, 200.0])

    # add some obstacles
    obs1 = Block(60.0, 30.0, 120.0, 100.0, 140.0, clr=[0.4, 0.5, 0.4])
    obs2 = Block(60.0, 40.0, 180.0, 170.0, 50.0, clr=[0.5, 0.5, 0.6])
    obs3 = Block(40.0, 60.0, 90.0, 30.0, 70.0, clr=[0.3, 0.3, 0.4])
