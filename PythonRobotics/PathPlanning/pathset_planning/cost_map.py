...
class PassageMap(object):
    ...
    def add_obstacle(
        self, size_x, size_y, pos_x, pos_y, color="b", is_wall: bool = False
    ):
        """
    ...
    def filter_passage(self):
        ...
        for passage in self._passages:
            ...
            cylinder_t = fcl.Transform(
                jnp.array([passage_center[0], passage_center[1], 0.0])
            )
    ...
    def get_path_intersection(self, path):
        ...
        for i in range(path_len - 1):
            for passage in self._filtered_passages:
                ...
                extended_vrtx1 = (
                    jnp.array([passage.vrtx1[0], passage.vrtx1[1]])
                    + 250.0 * direction_n
                )
                extended_vrtx2 = (
                    jnp.array([passage.vrtx2[0], passage.vrtx2[1]])
                    + 250.0 * direction_p
                )
    ...
    def visualize(self, ax):
        ...
        for obs in self._obstacle:
            plot_rectangle(
                obs._size_x, obs._size_y, obs._pos_x, obs._pos_y, ax, color=obs._clr
            )
    ...
    def __repr__(self) -> str:
        return (
            "PassageMap"
            + self._name
            + f" with #{len(self._obstacle)} obstacles"
...
class CityCostMapLayer(ABCCostMapLayer):
    ...
    def __init__(self, city_map: CityMap, k: float = -100.0):
        self.name = "city"

        self._city_map = city_map
        self._k = k

        zs = [0.0] + [block._size_z for block in city_map._obstacle] + [200.0]
    ...
    def compute_edge_cost(self, parent_node: Node, child_node: Node):
        ...
        for passage_map in self._passage_maps:
            if height >= passage_map._region[0] and height < passage_map._region[1]:
                passage = passage_map.check_passage_intersection(
                    parent_node, child_node
                )
                if passage is None:
                    # no intersection
                    edge_cost = (
                        0.0  # self._k * (parent_node.min_dist - parent_node.min_dist)
                    )
                    ...
                    if passage.min_dist < parent_node.min_dist:
                        ...
                        edge_cost = 0.0  # self._k * parent_node.min_dist
                        passage_len = parent_node.min_dist
