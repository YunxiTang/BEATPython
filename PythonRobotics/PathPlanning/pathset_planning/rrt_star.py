...
class RRTStar(RRT):
    ...
    def __init__(self, connect_range, *args, **kwargs) -> None:
        ...
        self._gamma_rrt_star = (
            2.0 * (1.0 + 1.0 / self._d) ** (1.0 / self._d) * (1.0) ** (1.0 / self._d)
        )
    ...
    def _compute_connective_range(self):
        return jnp.minimum(
            self._gamma_rrt_star
            * (jnp.log(self._card()) / self._card()) ** (1.0 / self._d),
            self._connect_range,
        )
    ...
    def _find_near_node_idx(self, new_node: Node):
        ...
        """
        node_dists = [
            RRT._compute_node_distance(new_node, node) for node in self._node_list
        ]
        near_idxs = [
            node_dists.index(i)
            for i in node_dists
            if i <= self._compute_connective_range()
        ]
        return near_idxs
    ...
    def _choose_parent(self, near_node_idxs: List[int], new_node: Node):
        ...
        """
        costs = [
            self._node_list[idx].cost
            + self._costMap.compute_edge_cost(self._node_list[idx], new_node)
        ...
        if costs:
            ...
            for idx in sorted_idx:
                if not self._check_edge_collision(
                    self._node_list[near_node_idxs[idx]], new_node
                ):
                    new_node.reset_parent(self._node_list[near_node_idxs[idx]])
                    break

        new_node_cost = new_node.parent.cost + self._costMap.compute_edge_cost(
    ...
    def _rewire(self, new_node: Node, near_node_idxs: List[int]):
        ...
        for idx in near_node_idxs:
            ...
            if new_node.parent != node:
                updated_cost = new_node.cost + self._costMap.compute_edge_cost(
                    new_node, node
                )
                if updated_cost < node.cost and (
                    not self._check_edge_collision(node, new_node)
                ):
    ...
    def plan(self, verbose=True, animation=True, early_stop=True):
        ...
        for i in range(self._max_iter):
            ...
            if verbose and i % 10 == 0:
                ...
                print(
                    self._gamma_rrt_star
                    * (jnp.log(self._card()) / self._card()) ** (1.0 / self._d),
                    self._compute_connective_range(),
                )
        ...
        if not self._check_edge_collision(self._potential_final_node, self._goal):
            self._goal.set_parent(self._potential_final_node)
            self._goal.set_cost(
                self._potential_final_node.cost
                + self._costMap.compute_edge_cost(
                    self._potential_final_node, self._goal
                )
            )
            sol = self._generate_final_course()
            print(
                f"Find a path with {len(sol)} nodes due to max_iter. Goal cost: {self._goal.cost}"
            )
        else:
...
class CityRRTStar(RRT):
    ...
    def __init__(self, connect_range, *args, **kwargs) -> None:
        ...
        self._gamma_rrt_star = (
            2.0 * (1.0 + 1.0 / self._d) ** (1.0 / self._d) * (50.0) ** (1.0 / self._d)
        )
        self._yita = self._step_size
    ...
    def _choose_parent(self, near_node_idxs: List[int], new_node: Node):
        ...
        costs = [
            self._node_list[idx].cost
            + self._costMap.compute_edge_cost(self._node_list[idx], new_node)[0]
            for idx in near_node_idxs
        ]
        ...
        if costs:
            ...
            for idx in sorted_idx:
                if not self._check_edge_collision(
                    ...
                    new_node.reset_parent(self._node_list[near_node_idxs[idx]])
                    break
        edge_cost, aux_infos = self._costMap.compute_edge_cost(
            new_node.parent, new_node
        )
        ...
        for i in range(self._max_iter):
            ...
            if verbose and i % interval == 0:
                ...
                if tmp_near_node_idxs:
                    ...
                    print(
                        f"Iter: {i} || No. of Tree Nodes: {len(self._node_list)} || Cost: {tmp_new_node.cost} || PW: {tmp_new_node.min_dist}"
                    )
                    if animation:
            ...
            self._connect_range = (
                0.9995 * self._connect_range if self._connect_range > 10.0 else 10.0
            )
