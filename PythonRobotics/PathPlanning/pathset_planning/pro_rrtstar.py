...
class Node(object):
    ...
    def __init__(self, state):
        ...
        # For usage in RRT*
        self._cost = -100.0 * 200.0
        self.min_dist = 200.0

    def set_parent(self, node):
        self._parent = node  # copy.deepcopy(node)
    ...
    def __repr__(self):
        return "RRT Node @ [State({}), Cost({})]".format(self.state, self.cost)


def compute_node_distance(node1: Node, node2: Node):
    return jnp.linalg.norm(node1.state - node2.state)
...
class PassageAwareRRTStar:
    ...
    def __init__(
        ...
        self._step_size = (
            step_size if step_size > self._resolution else 2.0 * self._resolution
        )
    ...
    def _check_edge_collision(self, node1: Node, node2: Node) -> bool:
        return self._world_map.check_line_collision(node1.state, node2.state)

    def _check_node_collision(self, node: Node):
        return self._world_map.check_pos_collision(node.state)
    ...
    def _steer(self, from_node: Node, to_node: Node, step_size: float) -> Node:
        ...
        if dist <= step_size:
            ...
            new_node_state = (
                from_node.state + step_size * (to_node.state - from_node.state) / dist
            )
    ...
    def _choose_parent(self, near_nodes: List[Node], new_node: Node):
        ...
        """
        costs = [
            near_node.cost + self._costMap.compute_edge_cost(near_node, new_node)[0]
    ...
    def _rewire(self, new_node: Node, near_nodes: List[Node]):
        ...
        In such a case, this will re-assign the parent of the near nodes to new_node.
        """
    ...
    def make_animation(self, path_solution):
        ...
        tmp_list = list(self._node_tree.inorder())
        ...
        for i in range(self._max_iter):
            ...
            if verbose and i % interval == 0:
                ...
                if tmp_near_nodes:
                    ...
                    print(
                        f"Iter: {i} || Cost: {tmp_new_node.cost} || PW: {tmp_new_node.min_dist}"
                    )
