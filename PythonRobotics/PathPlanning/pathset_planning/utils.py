...
def transfer_path(pivot_path, start, goal, delta_start=None, delta_goal=None):
    ...
    # tmp_node = SimpleNode(start, 0.0)
    # transfered_path.append(tmp_node)

    for i in range(pivot_path_num):
        tmp_state = (
            pivot_path[i].state
            + pivot_path[i].path_len / pivot_path_len * (delta_goal - delta_start)
            + delta_start
        )
...
def plot_rectangle(size_x, size_y, pos_x, pos_y, ax, center=True, color="b"):
    if center:
        ax.add_patch(Rectangle([pos_x, pos_y], size_x / 2.0, size_y / 2.0, color=color))
        ax.add_patch(
            Rectangle((pos_x, pos_y), -size_x / 2.0, size_y / 2.0, color=color)
        )
        ax.add_patch(
            Rectangle((pos_x, pos_y), -size_x / 2.0, -size_y / 2.0, color=color)
        )
...
if __name__ == "__main__":
    ...
    R = get_intersection_point(Point(0.6, 0.6), Point(1, 1), Point(0, 1), Point(1, 0))
    if R:
