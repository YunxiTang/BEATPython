    ...
    def sample_pos(self, rng_key, toward_goal=False):
        if not toward_goal:
            x = jax.random.uniform(
                rng_key, shape=(1,), minval=self.x_min, maxval=self.x_max
            )
...
class Node:
    ...
    def __init__(self, state: State):
        ...
        self._node_cost = (
            None  # cost caused by node itself, depends on the motion modality
        )
        self._path_cost = (
            None  # cost from root node to current node (include the node_cost)
        )
