...
from components import (
    ...
    WorldMap,
    Modality,
    EuclideanCostMapLayer,
    ModalityCostMapLayer,
)
...
class ModalityRRTStar:
    ...
    def _get_nearest_node(self, rand_node: Node):
        ...
        """
        dlist = [
            ModalityRRTStar._compute_node_distance(rand_node, node)
    ...
    def _steer(self, from_node: Node, to_node: Node, step_size: float) -> Node:
        ...
        if dist <= step_size:
            ...
            new_node_state_x = (
                from_node._state.x
                + step_size * (to_node._state.x - from_node._state.x) / dist
            )
    ...
    def _find_near_node_idx(self, new_node: Node):
        ...
        """
        node_dists = [
            ModalityRRTStar._compute_node_distance(new_node, node)
            for node in self._node_list
        ]
        near_idxs = [
            node_dists.index(i)
    ...
    def _choose_parent(self, near_node_idxs: List[int], new_node: Node):
        ...
        new_node_path_cost = (
            new_node.node_cost
            + new_node.parent.path_cost
            + self._costMap.compute_edge_cost(new_node.parent, new_node)[0]
        )
        ...
        for idx in near_node_idxs:
            ...
            if new_node.parent != node:
                updated_cost = (
                    new_node.path_cost
                    + node.node_cost
                    + self._costMap.compute_edge_cost(new_node, node)[0]
                )
    ...
    def plan(self, verbose=True, early_stop=True):
        ...
        print(
            f"Find a path with {len(sol)} nodes due to max_iter. Goal cost: {last_node.path_cost * self._step_size}"
        )
