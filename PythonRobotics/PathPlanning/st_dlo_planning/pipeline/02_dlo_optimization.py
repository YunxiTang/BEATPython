if __name__ == "__main__":
    ...
    result_path = pathlib.Path(__file__).parent.parent.joinpath(
        "results", map_cfg_file.logging.save_pivot_path_name
    )
    ...
    for kk in range(pivot_path.shape[0] - 1):
        ax[0].plot(
            [pivot_path[kk, 0], pivot_path[1 + kk, 0]],
            [pivot_path[kk, 1], pivot_path[1 + kk, 1]],
            "r-",
        )
    ...
    zarr_root = zarr.open(
        "/home/yxtang/CodeBase/PythonCourse/PythonRobotics/PathPlanning/st_dlo_planning/results/gdm_mj/train/task03_10.zarr"
    )
    ...
    for k in range(num_path):
        newpath = transfer_path_between_start_and_goal(
            pivot_path, scaled_delta_starts[k], scaled_delta_goals[k]
        )
    ...
    _, _, new_pathset_list, backup_pathset_list = deform_pathset_step1(
        pivot_path, pathset_list, world_map, 0.15
    )
    final_pathset, SegIdx = deform_pathset_step2(
        np.array(backup_pathset_list), np.array(new_pathset_list)
    )
    ...
    for i in range(num_kp - 1):
        ax[1].plot(
            [init_dlo_shape[i][0], init_dlo_shape[i + 1][0]],
            [init_dlo_shape[i][1], init_dlo_shape[i + 1][1]],
            color="k",
            linewidth=3.0,
        )
        ax[1].plot(
            [goal_dlo_shape[i][0], goal_dlo_shape[i + 1][0]],
            [goal_dlo_shape[i][1], goal_dlo_shape[i + 1][1]],
            color="r",
            linewidth=3.0,
        )
    ...
    for s_path in polished_pathset:
        # initial shape
        ax[1].scatter(
            init_dlo_shape[p][0], init_dlo_shape[p][1], s=15, color=clrs[p], marker="o"
        )
        ax[1].scatter(
            goal_dlo_shape[p][0], goal_dlo_shape[p][1], s=15, color=clrs[p], marker="o"
        )

        for i in range(s_path.shape[0] - 1):
            ax[1].plot(
                [s_path[i, 0], s_path[i + 1, 0]],
    ...
    for i in range(pivot_path.shape[0] - 1):
        ax[1].plot(
            [pivot_path[i, 0], pivot_path[i + 1, 0]],
            [pivot_path[i, 1], pivot_path[i + 1, 1]],
            "k-.",
            linewidth=1.0,
        )
    ...
    solver = TcDloSolver(
        pathset=pathset, k1=0.47836547, k2=2.18431412, tol=1e-5, max_iter=2500
    )
    ...
    for i in range(0, pathset.T + 1, 1):
        ...
        dlo_shape = polish_dlo_shape(
            dlo_shape, k1=0.47836547, k2=2.18431412, segment_len=seg_len
        )
    ...
    for i in range(0, pathset.T + 1, 4):
        ...
        ax[2].plot(
            polished_dlo_shape[:, 0],
            polished_dlo_shape[:, 1],
            color=[clrs[i], rever_clrs[i], clrs[i]],
            linewidth=3,
        )
    ...
    plt.savefig(
        f"/home/yxtang/CodeBase/PythonCourse/PythonRobotics/PathPlanning/st_dlo_planning/results/transfered_path_{map_case}.png",
        dpi=1200,
    )
    plt.axis("equal")
    plt.show()
    ...
    plt.show()
    ani.save(
        filename=f"/home/yxtang/CodeBase/PythonCourse/PythonRobotics/PathPlanning/st_dlo_planning/results/optimized_{map_case}.gif",
        writer="pillow",
    )
