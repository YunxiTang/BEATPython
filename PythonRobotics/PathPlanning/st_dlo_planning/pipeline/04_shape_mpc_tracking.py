if __name__ == "__main__":
    ...
    from st_dlo_planning.utils.world_map import (
        Block,
        WorldMap,
        MapCfg,
        load_mapCfg_to_mjcf,
    ...
    result_path = pathlib.Path(__file__).parent.parent.joinpath(
        "results", "optimal_shape_seq_res", f"{map_case}_optimal_shape_seq.npy"
    )
    ...
    # @torch.jit.script
    def path_cost_func_tensor(
        dlo_kp_2d: torch.Tensor,
        eef_states: torch.Tensor,
        u: torch.Tensor,
        target_dlo_kp_2d: torch.Tensor,
        phi,
    ):
        ...
        weight_curve = (
            torch.diag(
                torch.tensor(
                    [1.0, 1.0] + [1.0, 1.0] * (13 - 2) + [1.0, 1.0], device=device
                )
            )
            * 150.0
        )
        ...
        for ofo in obs_info:
            ...
            for i in range(dlo_kp_2d.shape[0]):
                dlo_kp = dlo_kp_2d[i]
                sdf1 = torch.dot(
                    dlo_kp - vertexs[0], norm_vecs[0] / torch.norm(norm_vecs[0])
                )
                sdf2 = torch.dot(
                    dlo_kp - vertexs[1], norm_vecs[1] / torch.norm(norm_vecs[1])
                )
                sdf3 = torch.dot(
                    dlo_kp - vertexs[2], norm_vecs[2] / torch.norm(norm_vecs[2])
                )
                sdf4 = torch.dot(
                    dlo_kp - vertexs[3], norm_vecs[3] / torch.norm(norm_vecs[3])
                )

                full_sdf = torch.concatenate(
                    (sdf1[None], sdf2[None], sdf3[None], sdf4[None]), dim=0
                )
            ...
            for m in range(2):
                ...
                sdf1 = torch.dot(
                    eef_pos - vertexs[0], norm_vecs[0] / torch.norm(norm_vecs[0])
                )
                sdf2 = torch.dot(
                    eef_pos - vertexs[1], norm_vecs[1] / torch.norm(norm_vecs[1])
                )
                sdf3 = torch.dot(
                    eef_pos - vertexs[2], norm_vecs[2] / torch.norm(norm_vecs[2])
                )
                sdf4 = torch.dot(
                    eef_pos - vertexs[3], norm_vecs[3] / torch.norm(norm_vecs[3])
                )
        ...
        left_theta_target = wrap_angle(
            angle_with_x_axis(target_dlo_kp_2d[1], target_dlo_kp_2d[0])
        )
        right_theta_target = wrap_angle(
            angle_with_x_axis(target_dlo_kp_2d[-1], target_dlo_kp_2d[-2])
        )
        ...
        orien_loss = (
            (left_theta_target - left_theta) ** 2
            + (right_theta_target - right_theta) ** 2
            + 0
        )  # torch.sum((eef_states[0, 0:2] - target_dlo_kp_2d[0]) ** 2) + torch.sum((eef_states[1, 0:2] - target_dlo_kp_2d[-1]) ** 2)
        c = position_loss + curve_loss + ctrl_loss + 5e-3 * obs_loss + orien_loss * 0.5
        return c

    # @torch.jit.script
    def final_cost_func_tensor(
        dlo_kp_2d: torch.Tensor,
        eef_states: torch.Tensor,
        target_dlo_kp_2d: torch.Tensor,
        phi,
        ...
        for ofo in obs_info:
            ...
            for i in range(dlo_kp_2d.shape[0]):
                ...
                if sdf_val < 0.05:
                    obs_loss = obs_loss + relaxed_log_barrier(
                        sdf_val, phi=phi
                    )  # 1 / (sdf_val**4)
    ...
    env = DualGripperCableEnv(
        task=task_id, feat_stride=3, render_mode=render_mode, camera_name=camera
    )
    obs, _ = env.reset()
    ...
    buffer = ReplayBuffer(
        obs_dim=env.num_feat * 3 + env.num_grasp * 3, act_dim=env.num_grasp * 3
    )
    ...
    model_params = torch.load(model_ckpt_path, pickle_module=dill)["state_dicts"][
        "model"
    ]
    ...
    while i < T:
        ...
        if ref_idx == 0:
            ...
            delta_eef, obj_loss, hmin = agent.select_action(
                dlo_kp, eef_states, dlo_kp_ref, safe_mode=False
            )
        else:
            ...
            #         ref_idx += 1
            delta_eef, obj_loss, hmin = agent.select_action(
                dlo_kp, eef_states, dlo_kp_ref, safe_mode=True
            )
        ...
        if hmin > 0.02:
            ...
            num_step = 1
        next_obs, reward, done, truncated, info = env.step_relative_eef(
            action[0:3], action[3:], num_steps=num_step, render_mode=render_mode
        )
        ...
        if i % 5 == 0:
            print(
                f"env_step: {i}, ref_idx {ref_idx}/{planned_shape_seq.shape[0]}, inter_err: {error:.4f}, ultra_err: {ultra_error:.4f}, hmin: {cons_vio:.4f}, patience: {patience}, err_patience: {error_patience}"
            )
        ...
        if (
            (error < 4e-3 and ref_idx < planned_shape_seq.shape[0] - 1)
            or (error_patience > 15)
            or (patience > 150 and ref_idx > 0)
        ):
            ref_idx += 1
    ...
    ax_ani.plot(
        final_target_shape[:, 0],
        final_target_shape[:, 1],
        color="r",
        linewidth=1.5,
        marker="o",
        mec="r",
    ...
    for i in range(0, len(ani_dlos), 3):
        container1 = ax_ani.plot(
            ani_dlos[i][:, 0],
            ani_dlos[i][:, 1],
            color=[clrs[i], rever_clrs[i], clrs[i]],
            linewidth=2,
        )
    ...
    artists.append(container1)
    containerf = ax_ani.plot(
        ani_dlos[-1][:, 0],
        ani_dlos[-1][:, 1],
        color=[clrs[-1], rever_clrs[-1], clrs[-1]],
        linewidth=1.5,
        marker="o",
        mec="r",
        mfc="r",
