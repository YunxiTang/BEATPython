...
class DloOmpl:
    def __init__(
        ...
        state_validality_checker = MyStateValidityChecker(
            self.space_info, self.world_map
        )
        ...
        self.problem_def.setIntermediateSolutionCallback(
            ob.ReportIntermediateSolutionFn(call_back)
        )

        # ======== step 7: set the optimization objective
        pathLengthObjective = ob.PathLengthOptimizationObjective(self.space_info)
        passageWidthObjective = PassageOptimizationObjective(
            self.space_info, self.world_map
        )
        ...
        self.stop_criteria = ob.CostConvergenceTerminationCondition(
            self.problem_def, solutionsWindow=10, epsilon=0.1
        )
    ...
    def plan(
        ...
        if solved:
            ...
            for i in range(sol_path_geometric.getStateCount()):
                ...
                states.append([state[0], state[1], state[2]])
            sol_np = np.array(states)
            vec_smooth_traj_fn = jax.vmap(
                query_point_from_path, in_axes=[0, None, None]
            )
...
class PassageOptimizationObjective(ob.OptimizationObjective):
    ...
    def motionCost(self, state1, state2):
        ...
        if nd > 1:
            ...
            for j in range(nd - 1):
                self.space_info.getStateSpace().interpolate(
                    state1, state2, j / nd, test_state1
                )
                self.space_info.getStateSpace().interpolate(
                    state1, state2, (j + 1) / nd, test_state2
                )
                test_state1_np = self._get_array_from_state(test_state1)
                test_state2_np = self._get_array_from_state(test_state2)

                passage = self.world_map.check_passage_intersection(
                    test_state1_np, test_state2_np
                )
                if passage is not None:
                    passage_width = passage.min_dist
                    in_cost = 1.0 / (passage_width**2)
                else:
                    in_cost = 1e-5
            ...
            if passage is not None:
                passage_width = passage.min_dist
                incremental_Cost = 1.0 / (passage_width**2)
            else:
                incremental_Cost = 1e-5
