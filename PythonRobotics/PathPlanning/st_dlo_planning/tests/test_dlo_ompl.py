if __name__ == "__main__":
    ...
    from PythonRobotics.PathPlanning.st_dlo_planning.utils.world_map import (
        Block,
        WorldMap,
        MapCfg,
    )

    map_cfg = MapCfg(map_zmin=0, map_zmax=100.0)

    world_map = WorldMap(map_cfg)
    # ============== add some obstacles =========================
    size_z = 100

    world_map.add_obstacle(Block(40.0, 40.0, size_z, 156.0, 43.5, clr=[0.3, 0.5, 0.4]))

    world_map.add_obstacle(Block(26.0, 15.0, size_z, 29.0, 57.0, clr=[0.3, 0.3, 0.4]))

    world_map.add_obstacle(Block(24.0, 15.0, size_z, 85.0, 80.0, clr=[0.2, 0.6, 0.4]))

    world_map.add_obstacle(Block(20.0, 20.0, size_z, 39.0, 132.0, clr=[0.6, 0.6, 0.8]))

    world_map.add_obstacle(Block(28.0, 15.0, size_z, 141.0, 81.0, clr=[0.6, 0.1, 0.4]))
