if __name__ == "__main__":
    ...
    from st_dlo_planning.temporal_config_opt.opt_solver import (
        DloOptProblem,
        TcDloSolver,
    )
    from st_dlo_planning.utils import PathSet

    waypoints1 = jnp.array(
        [[0.28, 0.1], [0.4, 0.45], [0.35, 1.0], [0.25, 1.42]]
    ) + jnp.array([[0.03, 0.0]])
    waypoints2 = jnp.array([[0.53, 0.0], [0.52, 0.5], [0.48, 1.0], [0.47, 1.40]])
    waypoints3 = jnp.array([[0.75, 0.0], [0.6, 0.55], [0.6, 1.0], [0.68, 1.5]])
    waypoints4 = jnp.array(
        [[0.75, 0.0], [0.6, 0.55], [0.6, 1.0], [0.68, 1.5]]
    ) + jnp.array([[0.12, 0.1]])
    waypoints5 = jnp.array(
        [[0.75, 0.0], [0.6, 0.55], [0.6, 1.0], [0.68, 1.5]]
    ) + jnp.array([[0.22, 0.15]])
    waypoints6 = jnp.array(
        [[0.75, 0.0], [0.6, 0.55], [0.6, 1.0], [0.68, 1.5]]
    ) + jnp.array([[0.32, 0.2]])
    ...
    plt.plot(
        np.linspace(0.0, 1.0, pathset.T + 1, endpoint=True),
        np.linspace(0.0, 1.0, pathset.T + 1),
        "r-.",
    )
    plt.axis("equal")
    plt.show()
