...
if __name__ == '__main__':
    ...
    for i in range(0, len(dlo_keypoints_hist), 5):
        ...
        ax.scatter(l_eef_pos[0], l_eef_pos[1], l_eef_pos[2], color='k',  marker='d', s=45)
        ax.scatter(r_eef_pos[0], r_eef_pos[1], r_eef_pos[2], color='k',  marker='d', s=45)
    ...
    while i < N:
        ...
        # ================== log printing ====================== #
        if i % 20 == 0:
            print(f'Timestep: [{i} / {N}] || Shape Error: {error} Shape Error Reduction: {error_pre-error}')
            error_pre = error
        osb = next_obs
        pre_action = action
        i += 1
