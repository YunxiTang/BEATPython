...
class GradientLMPCAgent:
    def __init__(self, 
        ...
        #     lr=self.lr,
        #     weight_decay=1e-5 )
        self.model_optimizer = optim.AdamW(self.gdm_model.parameters(), lr=self.lr, weight_decay=1e-5 )
    ...
    def update_local_Jacobian(self, state, action, next_state):
        ...
        self.local_jacobian += 1.0 * (delta_x - self.local_jacobian @ delta_y) / (np.sum(delta_y**2)) @ delta_y.T
    ...
    def update(self, transition_dict):
        ...
        dlo_keypoints = states_tensors[:, 0:self.num_feats*3].reshape(-1, self.num_feats, 3)
        eef_states = states_tensors[:, self.num_feats*3:].reshape(-1, 2, 3)

        next_dlo_keypoints = next_states_tensors[:, 0:self.num_feats*3].reshape(-1, self.num_feats, 3)
