...
class GDMTrainer(BaseTrainer):
    ...
    def run(self, rank, world_size):
        ...
        val_batch_size = cfg.val_dataloader.batch_size // world_size
        test_batch_size = cfg.test_dataloader.batch_size // world_size
        ...
        val_dataloader = DataLoader(val_dataset, batch_size=val_batch_size, sampler=val_sampler)
        test_dataloader = DataLoader(test_dataset, batch_size=test_batch_size, sampler=test_sampler)
...
def main_worker(rank, world_size, cfg, gdm_cfg):
    trainer = GDMTrainer(cfg, gdm_cfg)
    trainer.run(rank, world_size)

def main():
    cfg = OmegaConf.load('path_to_config.yaml')
    gdm_cfg = GDM_CFG()
    world_size = torch.cuda.device_count()
    mp.spawn(main_worker, args=(world_size, cfg, gdm_cfg), nprocs=world_size, join=True)
