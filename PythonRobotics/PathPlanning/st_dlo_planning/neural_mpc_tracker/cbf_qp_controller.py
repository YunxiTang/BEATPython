...
barrier_func_grad = jax.jit( jax.grad(barrier_func_tmp, argnums=[0,]) )
...
class MultiCBFQP:
    ...
    def _get_h_value(self, dlo_kp:np.ndarray):
        ...
        for i in range(self.num_obs):
            hs_i = vmap(barrier_func, in_axes=[0, None])(jau.from_numpy(x), jau.from_numpy(self.obs_vertixs[i]))
    ...
    def _get_h_grad(self, dlo_kp:np.ndarray):
        ...
        x_shaped = dlo_kp
        for i in range(self.num_obs):
            tmp_res, = vmap(barrier_func_grad, in_axes=[None, 0, None])(jau.from_numpy(x_shaped), jnp.arange(0, self.num_feat), jau.from_numpy(self.obs_vertixs[i]))
    ...
    def solve_control(self, 
        ...
        u_opt = solve_qp(P=P_qp, q=q_qp, G=G, h=h, lb=lb, ub=ub, solver="osqp", 
                         eps_abs=1e-3, initvals=self.Uinit, verbose=self.verbose)
