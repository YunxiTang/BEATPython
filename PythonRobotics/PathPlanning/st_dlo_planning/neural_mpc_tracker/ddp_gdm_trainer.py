...
from st_dlo_planning.neural_mpc_tracker.base_trainer import (
    BaseTrainer,
    ...
    def run(self, rank, world_size):
        ...
        for sub_data_dir in train_data_dirs:
            ...
            for train_data_path in train_data_paths:
                sub_dataset = MultiStepGDMDataset(
                    data_path=train_data_path, max_step=30
                )
        ...
        train_dataloader = prepare_dataloader(
            train_dataset, rank, world_size, train_batch_size
        )

        # configure logging
        if rank == 0:
            wandb_logger = DataLogger(
                log_dir=cfg.logging.output_dir, wandb_entity="tangyunxi000", config=cfg
            )
        ...
        for epoch_idx in range(max_epoch):
            ...
            if rank == 0:
                ...
                wandb_logger.log(
                    "ave_train_loss", train_loss / total_samples, self.epoch
                )
                wandb_logger.log("lr", self.scheduler.get_lr()[0], self.epoch)

            if (
                cfg.checkpoint.save_checkpoint
                and epoch_idx % cfg.checkpoint.checkpoint_every == 0
                and rank == 0
            ):
...
if __name__ == "__main__":
    ...
    mp.spawn(
        main_worker, args=(world_size, train_cfg, gdm_cfg), nprocs=world_size, join=True
    )
