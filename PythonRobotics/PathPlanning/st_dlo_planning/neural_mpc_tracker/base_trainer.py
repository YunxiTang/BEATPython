...
class CosineWarmupScheduler(optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        ...
        return [
            np.maximum(base_lr * lr_factor, self.min_lr) for base_lr in self.base_lrs
        ]
...
class BaseTrainer:
    ...
    def save_model_cfg(self, model_cfg_path=None):
        if model_cfg_path is None:
            model_cfg_path = os.path.join(
                self._cfg.checkpoint.ckpt_dir, f"model_cfg.yaml"
            )
    ...
    def save_data_stats(self, datastats: dict, datastats_path=None):
        ...
        if datastats_path is None:
            datastats_path = os.path.join(
                self._cfg.checkpoint.ckpt_dir, f"data_stats.yaml"
            )
    ...
    def save_checkpoint(
        ...
        payload = {"cfg": self._cfg, "state_dicts": dict(), "pickles": dict()}
        ...
        if use_thread:
            self._saving_thread = threading.Thread(
                target=lambda: torch.save(
                    payload, ckpt_path.open("wb"), pickle_module=dill
                )
            )
