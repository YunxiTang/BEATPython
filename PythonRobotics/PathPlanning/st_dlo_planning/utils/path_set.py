...
class PathSet:
    def __init__(self, all_path, T: int, seg_len: float):
        ...
        self.query_dlo_shape_fn = jax.jit(
            jax.vmap(query_point_from_path, in_axes=[0, 0])
        )
...
def transfer_path_between_start_and_goal(pivot_path, delta_start, delta_goal):
    ...
    for i in range(pivot_path_num):
        tmp_state = (
            pivot_path[i]
            + chord_distances[i] / pivot_arc_len * (delta_goal - delta_start)
            + delta_start
        )
...
def redistribute_points_with_ratios(
    points, original_start, original_end, new_start, new_end, max_dist=None
):
    """
    ...
    if max_dist:
        scale_factor = min(
            np.linalg.norm(new_end - new_start), max_dist
        ) / np.linalg.norm(new_end - new_start)
    else:
        scale_factor = 1.0
...
def redistribute_points_by_scale(
    points, original_start, original_end, new_start, new_end, max_dist
):
    """
    ...
    for i in range(num_path):
        directional_vec = (points[i] - middle_point) / np.linalg.norm(
            points[i] - middle_point
        )
        result[i] = middle_point + ratio * directional_vec * np.linalg.norm(
            points[i] - middle_point
        )
    return result


def deform_pathset_step1(
    pivot_path, pathset_list: List[np.ndarray], world_map: WorldMap, max_pw=None
):
    """
    ...
    for passage_num in range(num_passage):
        ...
        direction_p = np.array(
            [passage.vrtx2[0] - passage.vrtx1[0], passage.vrtx2[1] - passage.vrtx1[1]]
        )
        direction_p = direction_p / np.linalg.norm(direction_p)
        direction_n = np.array(
            [passage.vrtx1[0] - passage.vrtx2[0], passage.vrtx1[1] - passage.vrtx2[1]]
        )
        direction_n = direction_n / np.linalg.norm(direction_n)

        extended_vrtx1 = (
            np.array([passage.vrtx1[0], passage.vrtx1[1]]) + 0.0 * direction_n
        )
        extended_vrtx2 = (
            np.array([passage.vrtx2[0], passage.vrtx2[1]]) + 0.0 * direction_p
        )
        ...
        for path_num in range(num_path):
            ...
            cumulative_distances = np.concatenate(
                [jnp.array([0.0]), jnp.cumsum(distances)]
            )
            ...
            for i in range(num_waypoints - 1):
                ...
                if point:
                    ...
                    sigma = (
                        np.linalg.norm(point.to_numpy() - path_1.to_numpy())
                        + cumulative_distances[i]
                    ) / total_length
                    intersects_on_this_passage.append(
                        {
                            "passage": passage,
    ...
    for passage_id, each_passage_intersects in pathset_intersects.items():
        ...
        if np.sum(np.linalg.norm(raw_points_np - passage_vrtx1, axis=1)) < np.sum(
            np.linalg.norm(raw_points_np - passage_vrtx2, axis=1)
        ):
        ...
        passage_direction = (passage_end - passage_start) / np.linalg.norm(
        ...
        redistributed_points = redistribute_points_by_scale(
            raw_points,
            original_start,
            original_end,
            passage_start,
        ...
        pathset_and_passage_intersects[passage_id] = (
            raw_points,
    ...
    for passage_id, each_passage_intersects in pathset_intersects.items():
        for each_intersect in each_passage_intersects:
            ...
            new_waypoint_np = np.array(
                [new_waypoint_np[0], new_waypoint_np[1], z_const]
            )
            pathset_list[path_num] = np.insert(
                pathset_list[path_num],
    ...
    for passage_id, each_passage_intersects in pathset_intersects.items():
        for each_intersect in each_passage_intersects:
            ...
            backup_pathset_list[path_num] = np.insert(
                backup_pathset_list[path_num],
                path_waypoint_idx + 1 + insertion_counter[path_num],
                new_waypoint_np,
                axis=0,
            )
    ...
    return (
        pathset_intersects,
        pathset_and_passage_intersects,
        pathset_list,
        backup_pathset_list,
    )
        ...
        for i in range(backup_num_waypoint):
            if np.linalg.norm(new_pathset[k, i] - backup_pathset[k, i]) != 0:
                ...
                for j in range(segment_idx[-2][0], segment_idx[-1][0]):
                    ratio = (
                        cumulative_distances[j] / total_length - segment_idx[-2][1]
                    ) / (segment_idx[-1][1] - segment_idx[-2][1])
                    polished_pathset[k, j] = (
                        backup_pathset[k, j]
                        + ratio * (segment_idx[-1][2] - segment_idx[-2][2])
                        + segment_idx[-2][2]
                    )

        segment_idx.append((i, 1.0, np.zeros(3)))
        for j in range(segment_idx[-2][0], segment_idx[-1][0] + 1):
            ratio = (cumulative_distances[j] / total_length - segment_idx[-2][1]) / (
                segment_idx[-1][1] - segment_idx[-2][1]
            )
            polished_pathset[k, j] = (
                backup_pathset[k, j]
