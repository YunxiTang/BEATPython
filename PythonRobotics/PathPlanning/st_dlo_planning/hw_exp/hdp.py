if __name__ == "__main__":
    ...
    with open(start_yaml_path, "r") as f:
        start_data = fit_bspline(
            np.array(yaml.safe_load(f)["points"]), 10
        )  # + np.array([0.00, -0.02, 0])
        # start_data = start_data[::-1]

    goal_yaml_path = os.path.join(task_folder, "goal_kp_world_5.yaml")
    with open(goal_yaml_path, "r") as f:
        goal_data = fit_bspline(np.array(yaml.safe_load(f)["points"]), 10) + np.array(
            [0.01, -0.03, 0]
        )
    ...
    for i in range(start_data.shape[0] - 1):
        ax.plot(
            [start_data[i, 0], start_data[i + 1, 0]],
            [start_data[i, 1], start_data[i + 1, 1]],
            "b-",
        )
    for i in range(goal_data.shape[0] - 1):
        ax.plot(
            [goal_data[i, 0], goal_data[i + 1, 0]],
            [goal_data[i, 1], goal_data[i + 1, 1]],
            "r-",
        )
    ...
    for i in range(sol_np.shape[0] - 1):
        ax.plot(
            [sol_np[i, 0], sol_np[i + 1, 0]], [sol_np[i, 1], sol_np[i + 1, 1]], "r-"
        )
    ...
    _, _, new_pathset_list, backup_pathset_list = deform_pathset_step1(
        pivot_path, pathset_list, world_map, 0.04
    )
    ...
    for i in range(0, pathset.T + 1, 1):
        ...
        dlo_shape = polish_dlo_shape(
            dlo_shape, k1=2.0, k2=1.0, segment_len=seg_len, iter=10, verbose=True
        )
