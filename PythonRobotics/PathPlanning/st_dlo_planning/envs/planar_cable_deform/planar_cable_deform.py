...
TaskSceneNames = {
    "03": {
        "file": Path(PureWindowsPath(r"assets\\dual_hand_thin_03_mod.xml")),
        "dlo_len": 0.3,
        "cable_geom_num": 39,
    },
    "04": {
        "file": Path(PureWindowsPath(r"assets\\dual_hand_thin_04.xml")),
        "dlo_len": 0.4,
        "cable_geom_num": 49,
    },
    "05": {
        "file": Path(PureWindowsPath(r"assets\\dual_hand_thin_05.xml")),
        "dlo_len": 0.5,
        "cable_geom_num": 39,
    },
    "06": {
        "file": Path(PureWindowsPath(r"assets\\dual_hand_thin_06.xml")),
        "dlo_len": 0.6,
        "cable_geom_num": 39,
    },
}
...
class DualGripperCableEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, task: str, feat_stride: int, **kargs):
        if task in TaskSceneNames:
            XML_FILE = os.path.join(
                os.path.dirname(current_path), TaskSceneNames[task]["file"]
            )
        else:
        ...
        self.left_hand_jnt_idxs = [
            self._get_joint_index(joint_name)
            for joint_name in ["left_px", "left_py", "left_rz"]
        ]
        self.right_hand_jnt_idxs = [
            self._get_joint_index(joint_name)
            for joint_name in ["right_px", "right_py", "right_rz"]
        ]
    ...
    def _get_obs(self):
        ...
        geom_pos = [
            self.data.geom(geom_name).xpos.copy().ravel()
            for geom_name in self.cable_geom_names
        ]
    ...
    def terminted(self, obs):
        ...
        if (
            gripper_distance <= self.min_distance
            or gripper_distance >= self.max_distance
        ):
            done = True
    ...
    def get_eef_twist(self):
        left_eef_bodyId = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "left_hand"
        )
        right_eef_bodyId = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "right_hand"
        )

        left_eef_twsit = np.zeros(
            6,
        )
        right_eef_twist = np.zeros(
            6,
        )
    ...
    def step_relative_eef(
        ...
        for i in range(num_steps):
            ...
            L_pose, R_pose = self.get_lowdim_eef_state(obs)
            error = np.linalg.norm(L_target - L_pose) + np.linalg.norm(
                R_target - R_pose
            )
    ...
    def plot_rope(rope_vec, ax, clr, lns=None, ldw=2.5):
        ...
        for i in range(n - 1):
            sns.scatterplot(
                x=[rope_vec[i, 0], rope_vec[i + 1, 0]],
                y=[rope_vec[i, 1], rope_vec[i + 1, 1]],
                ax=ax,
                color=clr,
                s=20,
            )
            ax.plot(
                [rope_vec[i, 0], rope_vec[i + 1, 0]],
                [rope_vec[i, 1], rope_vec[i + 1, 1]],
                color=clr,
                linestyle=lns,
                linewidth=ldw,
            )
