...
from st_dlo_planning.utils.mj_utils import InteractiveRender, Renderer
import gym
import numpy as np
...
class BaseMujocoEnv(gym.Env):
    ...
    def __init__(
        ...
        ctrl_bounds = self.model.actuator_ctrlrange.copy().astype(np.float32)
        self.na = ctrl_bounds.shape[0]
    ...
    def _set_action_space(self):
        """
        This method sets the action space
        """
    ...
    def do_simulation(self, ctrl, n_frames):
        ...
        """
        # Check control input matches the control shape
        if np.array(ctrl).shape[0] != self.na:
...
class MujocoEnv(BaseMujocoEnv):
    ...
    def _get_viewer(self, mode, width=DEFAULT_SIZE, height=DEFAULT_SIZE):
        ...
        if self.viewer is None:
            if mode == "human":
                from st_dlo_planning.utils.mj_utils import Viewer
                ...
                from st_dlo_planning.utils.mj_utils import RenderContextOffscreen
