...
def elastic_enery(
    ...
    for i in range(num_feature - 1):
        U1 = (
            ...
            + k1
            / 2.0
            * (jnp.linalg.norm(dlo_shape[i + 1] - dlo_shape[i]) - segment_len) ** 2
        )
    ...
    for j in range(num_feature - 2):
        U2 = (
            ...
            + k2
            / 2.0
            * (jnp.linalg.norm(dlo_shape[j + 2] - dlo_shape[j]) - 2 * segment_len) ** 2
        )
    ...
    for j in range(num_feature - 3):
        U3 = (
            ...
            + k2
            / 2.0
            * (jnp.linalg.norm(dlo_shape[j + 3] - dlo_shape[j]) - 3 * segment_len) ** 2
        )
    ...
    for j in range(num_feature - 4):
        U4 = (
            ...
            + k2
            / 2.0
            * (jnp.linalg.norm(dlo_shape[j + 4] - dlo_shape[j]) - 4 * segment_len) ** 2
        )
    ...
    for j in range(num_feature - 5):
        U5 = (
            ...
            + k2
            / 2.0
            * (jnp.linalg.norm(dlo_shape[j + 5] - dlo_shape[j]) - 5 * segment_len) ** 2
        )
...
def polish_dlo_shape(
    raw_shape, k1, k2, segment_len, iter: int = 500, verbose: bool = False
):
