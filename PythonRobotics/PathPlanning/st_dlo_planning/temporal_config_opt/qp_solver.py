...
def elastic_enery(intermediate_points, init_intermediate_points, end_point1, end_point2, k1, k2, segment_len):
    ...
    for j in range(num_feature-3):
        U3 = U3 + k2/7. * (jnp.linalg.norm(dlo_shape[j+3] - dlo_shape[j]) - 3 * segment_len) ** 2
    
    reg = 2.0 * jnp.mean((intermediate_points - init_intermediate_points)**2)
    return U1 + U2 + U3 + reg
...
def polish_dlo_shape(raw_shape, k1, k2, segment_len):
    ...
    for i in range(200):
        ...
        intermidiate_points = intermidiate_points - 0.02 * grads
        if i % 5 == 0:
            print(f'{i}: {energy}')
