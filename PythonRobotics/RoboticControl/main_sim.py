...
if __name__ == "__main__":
    ...
    ik_controller = IKController(
        ur5_xml, rbt_name="UR5", ee_name="fake_pinch_link", dt=0.001
    )

    init_frame = ik_controller.get_ee_pos(q, qdot, t)
    pregrasp_frame = RigidTransform(
        RollPitchYaw(np.pi, 0.0, np.pi / 3), p=np.array([0.4, 0.3, 0.55])
    )
    grasp_frame = RigidTransform(
        RollPitchYaw(np.pi, 0.0, np.pi / 3), p=np.array([0.4, 0.3, 0.46])
    )
    target_frame = RigidTransform(
        RollPitchYaw(np.pi, 0.0, np.pi / 2), p=np.array([0.5, -0.1, 0.6])
    )

    key_frames, grip_frames, key_times = task_setting(
        [init_frame, pregrasp_frame, grasp_frame, target_frame],
        [t, t + 2, t + 6, t + 8],
    )
    ...
    for i in range(N):
        ...
        grip = (
            grip_frames["grasp0_frame"]
            if t <= key_times["grasp0_frame"] + 0.5
            else grip_frames["grasp1_frame"]
        )
        ...
        x_error = (
            robot.mj_data.site("pinch").xpos.ravel()
            - task_traj.GetPose(t).translation()
        )
