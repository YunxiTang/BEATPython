...
if __name__ == "__main__":
    ...
    ee_pos = box_xpos + np.array([0.0, 0.0, 0.07])
    ...
    for i in range(6):
        plt.plot(
            np.linspace(0, count, count) * env.dt, np.array(logger.qdot_hist).T[i, :]
        )

    plt.figure(3)
    for i in range(3):
        plt.plot(
            np.linspace(0, count, count) * env.dt, np.array(logger.x_error_hist).T[:]
        )
    plt.show()
