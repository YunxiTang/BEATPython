...
@wp.kernel
def apply_gravity(vel: wp.array(dtype=wp.vec3),
...
@wp.kernel
def predict_position(pos: wp.array(dtype=wp.vec3),
...
@wp.kernel
def solve_distance_constraint(pred: wp.array(dtype=wp.vec3),
                              inv_mass: wp.array(dtype=float),
...
@wp.kernel
def update_velocity(pos: wp.array(dtype=wp.vec3),
                    pred: wp.array(dtype=wp.vec3),
                    vel: wp.array(dtype=wp.vec3),
                    dt: float):
    i = wp.tid()
...
for i in range(num_points):
    builder.add_particle(pos=init_pos[i],
                         vel=(0.0, 0.0, 0.0),
...
for step in range(steps):
    # 重力
    wp.launch(apply_gravity, dim=num_points, inputs=[vel, inv_mass, gravity, dt], device=device)

    # 预测
    wp.launch(predict_position, dim=num_points, inputs=[pos, vel, pred, dt], device=device)

    # 多次迭代满足约束
    for _ in range(iters):
        wp.launch(solve_distance_constraint, dim=num_points - 1,
                  inputs=[pred, inv_mass, indices, rest_lengths, stiffness, dt], device=device)

    # 更新速度 & 位置
    wp.launch(update_velocity, dim=num_points, inputs=[pos, pred, vel, dt], device=device)
