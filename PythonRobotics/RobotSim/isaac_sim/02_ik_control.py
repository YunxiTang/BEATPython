...
if __name__ == "__main__":
    ...
    table_asset = gym.load_asset(sim, asset_root, asset_file, asset_options)
    ...
    for i in range(env_num):
        ...
        table_handle = gym.create_actor(
            env_handle, table_asset, table_pose, "table", i, 0
        )
        ...
        ur5_handle_1 = gym.create_actor(
            env_handle, ur5_asset, ur5_pose_1, "ur5e1", i, 0
        )
        ...
        ur5_handle_2 = gym.create_actor(
            env_handle, ur5_asset, ur5_pose_2, "ur5e2", i, 0
        )
    ...
    rigid_body_positions = rigid_body_state_tensor[:, 0:7].view(
        env_num, body_num * 2 + 1, 7
    )
    rigid_body_velocities = rigid_body_state_tensor[:, 7:13].view(
        env_num, body_num * 2 + 1, 6
    )
    ...
    ur5_1_jacobians_tensor = gymtorch.wrap_tensor(
        ur5_1_jacobians_desc
    )  # in shape of: (num_envs, 10, 6, num_dofs)
    ur5e_1_eef_jacobian_tensor = ur5_1_jacobians_tensor[:, eef_link_index - 1, :, :]

    steps = 0

    target_pos_0 = (
        torch.stack(env_num * [torch.tensor([-0.75, 1.5, 2.2])])
        .to("cuda:0")
        .view((env_num, 3))
    )
    ...
    target_quat = (
        torch.stack(
            ...
            * [
                torch.tensor(
                    [target_quat_s.x, target_quat_s.y, target_quat_s.z, target_quat_s.w]
                )
            ]
        )
        .to("cuda:0")
    ...
    # 获取eef的local index
    rigid_body_index = gym.get_actor_rigid_body_index(
        env, actor_handle, num_rigid_bodies - 1, gymapi.DOMAIN_ENV
    )
    ...
    while not gym.query_viewer_has_closed(viewer):
        offset = torch.tensor(
            [0.01 * np.cos(steps * 0.02), 0.01 * np.sin(steps * 0.02), 0.0],
            device="cuda:0",
            dtype=torch.float32,
        )[None]
