...
def sdf_2d_box(p: jnp.ndarray, geom_param: dict) -> float:
    ...
    d = jnp.abs(p[0:2]) - b
    return jnp.linalg.norm(jnp.maximum(d, 0.0)) + jnp.minimum(
        jnp.maximum(d[0], d[1]), 0.0
    )
...
if __name__ == "__main__":
    ...
    trans = jnp.array(
        [
            [jnp.cos(0.0 * pi / 4.0), -jnp.sin(0.0 * pi / 4.0), 0.0],
            [jnp.sin(0.0 * pi / 4.0), jnp.cos(0.0 * pi / 4.0), 0.0],
            [0, 0, 1],
        ]
    )

    box = {"box_transform": trans, "box_size": [0.5, 0.2]}

    ball = {"sphere_center": jnp.array([0.0, 0.0]), "sphere_size": 0.1}
